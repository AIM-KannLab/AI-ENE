...
def combine_structures(dataset, patient_id, data_type, mask_arr, path_to_reference_image_nrrd, binary, return_type, output_dir=""):
    """
    Combines single mask nrrd files into a single nrrd file. It will first combine all masks as integers (1,2,3..) with 0 being background. This ensures that there are no overlapping masks (e.g. same voxel is both lung and tumor). If binary==true, it will split them into binary arrays (this is likely what you model will need).
    Args:
