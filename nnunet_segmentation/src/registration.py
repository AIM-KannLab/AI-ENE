...
def nrrd_reg_rigid(patient_id, moving_img, output_dir, fixed_img, image_format):
    ...
    # Ensure deterministic behavior with REGULAR sampling by avoiding thread-order nondeterminism
    registration_method.SetMetricAsMattesMutualInformation(numberOfHistogramBins=50)
    # Deterministic sampling: use full sampling (NONE)
    # registration_method.SetMetricSamplingStrategy(registration_method.NONE)
    # Use deterministic fixed-grid sampling for speed and repeatability
    registration_method.SetMetricSamplingStrategy(registration_method.REGULAR)
    # registration_method.SetMetricSamplingStrategy(registration_method.RANDOM)

    # Fix the sampling RNG seed for repeatability (if supported by your SimpleITK)
    try:
        registration_method.SetMetricSamplingPercentage(0.05, 42)
    except TypeError:
        # Older SimpleITK without the (percentage, seed) overload
        registration_method.SetMetricSamplingPercentage(0.05)

    # RANDOM sampling option (example; non-deterministic unless seeded). Keep commented.
    # registration_method.SetMetricSamplingStrategy(registration_method.RANDOM)
    # try:
    #     registration_method.SetMetricSamplingPercentage(0.05, 42)  # percentage, seed
    # except TypeError:
    #     registration_method.SetMetricSamplingPercentage(0.05)
